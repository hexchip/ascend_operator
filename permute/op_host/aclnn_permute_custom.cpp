...
[[maybe_unused]] uint32_t NNOPBASE_PermuteCustom = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnPermuteCustomTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "PermuteCustom";
    char inputDesc[] = {1};
    char outputDesc[] = {1};
    char attrDesc[] = {1};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(input);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, input, 0));
    NNOPBASE_ASSERT_NOTNULL_RETVAL(perm);
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddIntArrayAttr(*executor, perm, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddOutput(*executor, out, 0));
    if (NnopbaseAddParamName != NULL) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "input", true));
