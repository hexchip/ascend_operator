...
int main(int argc, char **argv)
{
    ...
    size_t outputXShapeSize_1=inputXShape[0] * inputXShape[1] * inputXShape[2];
    size_t dataType=2;
    std::vector<aclFloat16> inputXHostData(inputXShape[0] * inputXShape[1] * inputXShape[2]);
    std::vector<aclFloat16> outputZHostData(outputZShape[0] * outputZShape[1] * inputXShape[2]);
    ...
    // 执行算子
    ret = aclnnPermuteCustom(workspaceAddr, workspaceSize, executor, stream);
    ...
    //写出数据
    WriteFile("../output/output_z.bin", *output1, outputXShapeSize_1*dataType);
